...
function getGameImageHTML(game){
    let gameImage = document.createElement('img')
        gameImage.setAttribute('src', game.background_image?game.background_image:'')
...
function getGameRatingHTML(game){
    ...
    gameRating.textContent = game.metacritic?game.metacritic:'-'
    return gameRating
}
...
function getGameGenresHTML(game){
    ...
    gameGenres.textContent = game.genres?game.genres.map(genre=>genre.name).join(', '):''
    return gameGenres
}
...
function getGamePlatformsHTML(game){
    ...
    gamePlatforms.textContent = game.platforms?game.platforms.map(platform=>platform.platform.name).join(', '):''
